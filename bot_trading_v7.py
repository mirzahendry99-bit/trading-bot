"""
╔══════════════════════════════════════════════════════════════════╗
║         ALTCOIN TRADING BOT v7.0 — Clean & Safe Edition         ║
║                                                                  ║
...
║  1. Load & recover orphan positions                             ║
║  2. Evaluasi posisi: SL / TP1 / TP2 / TIME EXIT / STALE ALERT  ║
║  3. Safety checks (daily loss, cooldown, block hours, BTC)      ║
║  4. Scan semua pair → score → eksekusi entry                    ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
RISK_PCT_FLOOR      = 0.03   # minimum risk saat loss streak
RISK_PCT_CAP        = 0.08   # maximum risk saat win streak
EQUITY_LOOKBACK     = 5      # jumlah trade terakhir untuk ECC
MIN_ORDER_USDT      = 10.0   # minimum order agar valid di Gate.io
MAX_ORDER_USDT      = 12.0   # cap order — buffer fee & reserved balance
...
# ── Safety guards ────────────────────────────────────────────────
MAX_DAILY_LOSS       = 5.0   # stop entry jika rugi > $5 hari ini
...
# ── Scanner thresholds ───────────────────────────────────────────
MIN_VOLUME_USDT     = 150_000  # volume minimum pair per hari (USDT)
...
def calc_order_size(equity: float, entry: float, sl: float, risk_pct: float) -> float:
    """
    Hitung order size (USDT) via position sizing:
    Risk amount / SL distance, diklem MIN dan MAX.
    Buffer 20% equity untuk fee & slippage.
    """
    sl_pct = abs(entry - sl) / entry
    if sl_pct <= 0:
        return MIN_ORDER_USDT
    tradeable  = equity * 0.80
    order_usdt = (tradeable * risk_pct) / sl_pct
    order_usdt = max(order_usdt, MIN_ORDER_USDT)
    order_usdt = min(order_usdt, MAX_ORDER_USDT)
    order_usdt = min(order_usdt, tradeable)
...
def run():
    ...
    # ── Step 7: Ambil pair & prioritaskan trending ────────────────
    ...
    if trending:
        all_pairs = trending + [p for p in all_pairs if p not in trending]
