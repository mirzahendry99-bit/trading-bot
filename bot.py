...
# GATE.IO DIRECT HTTP
# =====================
def gate_sign(method, path, query="", body=""):
    ts = str(int(time.time()))
    hashed_body = hashlib.sha512(body.encode()).hexdigest()
    msg = "\n".join([method, path, query, hashed_body, ts])
    sig = hmac.new(SECRET_KEY.encode(), msg.encode(), hashlib.sha512).hexdigest()
    return ts, sig
...
def gate_request(method, path, params=None, data=None):
    ...
    ts, sig = gate_sign(method, path, query, body)
    ...
    try:
        ...
        raise Exception(f"Gate.io HTTP {e.code}: {error_body}")


def gate_create_order(currency_pair, side, funds=None, amount=None):
    """
    Buat order langsung via HTTP.
    Buy: gunakan 'funds' (USDT).
    Sell: gunakan 'amount' (token).
    """
    payload = {
...
def gate_get_order(order_id, currency_pair):
    return gate_request("GET", f"/spot/orders/{order_id}", params={"currency_pair": currency_pair})
...
def do_buy(pair, usdt_balance):
    ...
    if buy_price <= 0:
        price     = gate_get_price(pair)
        buy_price = price
        if funds > 0 and price > 0:
            filled = round(funds / price, 6)
