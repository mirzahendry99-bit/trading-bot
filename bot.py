...
def do_buy(client, pair, usdt_balance):
    # Pakai funds (USDT) langsung — lebih reliable di Gate.io
    ...
    # Gate.io market buy dengan funds — tidak perlu hitung amount manual
    order = gate_api.Order(
        currency_pair=pair,
        type="market",
        side="buy"
    )
    # funds diset via attribute, bukan constructor
    ...
    # Ambil detail via get_order untuk hasil yang akurat
    try:
        detail    = client.get_order(str(result.id), pair)
        buy_price = float(getattr(detail, "avg_deal_price", None) or 0)
        filled    = float(getattr(detail, "filled_amount", None) or 0)
    except Exception as e:
        print(f"⚠️ get_order error: {e}")
        buy_price = float(getattr(result, "avg_deal_price", None) or 0)
        filled    = float(getattr(result, "filled_amount", None) or 0)

    # Fallback: hitung dari harga terkini
    if buy_price <= 0:
        buy_price = get_price(client, pair)
    if filled <= 0:
        if buy_price > 0:
