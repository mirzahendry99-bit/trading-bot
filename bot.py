...
║                                                                  ║
║  Upgrade dari v4.0 (Audit Fix — Elite Tier):                   ║
║                                                                  ║
...
def do_buy(client, order_usdt: float) -> tuple:
    ...
    price = float(gate_retry(client.list_tickers, currency_pair=PAIR)[0].last)
    if price <= 0:
    ...
    order  = gate_api.Order(currency_pair=PAIR, type="market", side="buy", amount=str(amount))
...
def do_buy_limit(client, order_usdt: float) -> tuple:
    ...
    # Cancel + partial fill check
    try:
        gate_retry(client.cancel_order, PAIR, order_id)
        log(f"❌ Limit timeout — cancel")
    except Exception as e:
    ...
    if len(layer_sizes) >= 2 and layer_2_price:
        ...
        try:
            ...
            if amount2 >= (min_amount or 0.00001):
                ...
                if res2:
                    # Poll singkat — 30 detik
                    deadline2 = time.time() + 30
                    while time.time() < deadline2:
                        time.sleep(5)
                        try:
                            st2 = gate_retry(client.get_order, PAIR, res2.id)
                        except Exception:
                            st2 = None
                        ...
                        # Timeout → cancel layer 2, lanjut tanpa
                        try:
                            gate_retry(client.cancel_order, PAIR, res2.id)
                        except Exception:
                            pass
                        log(f"  [L2] ⏭ Layer 2 timeout → skip, lanjut dengan L1")
        except Exception as e:
            ...
            if amount3 >= (min_amount or 0.00001):
                ...
                if res3:
                    ...
                    while time.time() < deadline3:
                        time.sleep(5)
                        try:
                            st3 = gate_retry(client.get_order, PAIR, res3.id)
                        except Exception:
                            st3 = None
                        ...
                        try:
                            gate_retry(client.cancel_order, PAIR, res3.id)
                        except Exception:
                            pass
...
def do_sell(client, amount: float, label: str = "SELL") -> float:
    ...
    order  = gate_api.Order(currency_pair=PAIR, type="market", side="sell", amount=str(sell_amount))
    result = gate_retry(client.create_order, order)
    if result is None:
...
def run():
    log("=" * 55)
    log("🚀 BTC TRADING BOT v5.0 START")
    log("=" * 55)
    ...
    if position:
        ...
        if current_price >= tp1_price and not tp1_hit:
            ...
            if still_bull and remaining > 0:
                ...
                clear_position()
                log(f"TP1 FULL EXIT | Total: ~${partial_profit + (profit2 if remaining > 0 else 0):.4f}")
            return
